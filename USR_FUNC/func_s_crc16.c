#include "func_s_crc16.h"



// --------------------------------------------------------------
// CRC16计算方法:使用2个256长度的校验表
// --------------------------------------------------------------
#ifdef CRC_16_POLYNOMIALS_0x8005
unsigned char const crc16_h_table_8005[] =                          // CRC 高位字节值表
{
	
};

unsigned char const crc16_l_table_8005[] =                          // CRC 低位字节值表
{
	
};
#endif

#ifdef CRC_16_POLYNOMIALS_0x1021
unsigned char const crc16_h_table_1021[] =                          // CRC 高位字节值表
{
	
};

unsigned char const crc16_l_table_1021[] =                          // CRC 低位字节值表
{
	
};
#endif

/********************CRC16校验程序********************/
//--输入：待校验的数据首字节地址,需要校验的数据字节长度
//--输出：16位CRC校验码
/************************/
//--使用方法：在数据发送阶段，用于生成需要发送的数据后面的16位校验码
//------------在数据接收阶段，将接收到的数据经过此运算看是否得到0，若为0则说明接收到的数据正确
/*****************************************************/
#ifdef CRC_16_POLYNOMIALS_0x8005
extern unsigned int FUNC_S_CRC16_MODBUS_CAL(unsigned char* pchMsg, unsigned int wDataLen)
{
	unsigned char chCRCHi = 0xFF; // 高CRC字节初始化
	unsigned char chCRCLo = 0xFF; // 低CRC字节初始化
	unsigned int wIndex; // CRC循环中的索引

	while (wDataLen--)
	{
	 // 计算CRC
	wIndex = chCRCLo ^ *pchMsg++ ;
	chCRCLo = chCRCHi ^ crc16_h_table_8005[wIndex];
	chCRCHi = crc16_l_table_8005[wIndex] ;
	}

	return ((chCRCHi << 8) | chCRCLo) ;
}

extern unsigned int FUNC_S_CRC16_MODBUS_CHECK(unsigned char* pchMsg, unsigned int wDataLen)
{
	unsigned char chCRCHi = 0xFF; // 高CRC字节初始化
	unsigned char chCRCLo = 0xFF; // 低CRC字节初始化
	unsigned int wIndex; // CRC循环中的索引

	while (wDataLen--)
	{
	 // 计算CRC
	wIndex = chCRCLo ^ *pchMsg++ ;
	chCRCLo = chCRCHi ^ crc16_h_table_8005[wIndex];
	chCRCHi = crc16_l_table_8005[wIndex] ;
	}

	return ((chCRCHi << 8) | chCRCLo) ;
}
#endif

#ifdef CRC_16_POLYNOMIALS_0x1021
extern unsigned int FUNC_S_CRC16_XMODEM_CAL(unsigned char* pchMsg, unsigned int wDataLen)
{
	unsigned char chCRCH = 0x00; // 高CRC字节初始化
	unsigned char chCRCL = 0x00; // 低CRC字节初始化
	unsigned int wIndex; // CRC循环中的索引

	while (wDataLen--)
	{
	 // 计算CRC
	wIndex = chCRCH ^ *pchMsg++ ;
	chCRCH = chCRCL^crc16_l_table_1021[wIndex] ;		
	chCRCL = crc16_h_table_1021[wIndex];

	}

	return ((chCRCH << 8) | chCRCL) ;
}

extern unsigned int FUNC_S_CRC16_XMODEM_CHECK(unsigned char* pchMsg, unsigned int wDataLen)
{
	unsigned char chCRCH = 0x00; // 高CRC字节初始化
	unsigned char chCRCL = 0x00; // 低CRC字节初始化
	unsigned int wIndex; // CRC循环中的索引

	while (wDataLen--)
	{
	 // 计算CRC
	wIndex = chCRCH ^ *pchMsg++ ;
	chCRCH = chCRCL^crc16_l_table_1021[wIndex] ;		
	chCRCL = crc16_h_table_1021[wIndex];

	}

	return ((chCRCH << 8) | chCRCL) ;
}
#endif


void func_s_crc16_test1()
{
	static unsigned short int crc16_result[256];
	unsigned short int crc_count=0;
	for(crc_count=0;crc_count<256;crc_count++)
	{
		crc16_result[crc_count]=FUNC_S_CRC16_XMODEM_CAL((unsigned char*)&crc_count, 1);
	}
}




void func_s_crc16_test2()
{
	static unsigned char crc16_test_list[10]={1,6,3,4,5,6,7,8,9,10};
	unsigned short int crc_result=0;

	crc_result=FUNC_S_CRC16_XMODEM_CAL(&crc16_test_list[0], sizeof(crc16_test_list));
	crc_result=0;
}

